...
Quaternion::Quaternion(float eulerX, float eulerY, float eulerZ) {
  ...
  eulerY *= DEG2RAD;
  eulerZ *= DEG2RAD;
  Quaternion roll(sin(eulerX * 0.5f), 0, 0, cos(eulerX * 0.5f));
  Quaternion pitch(0, sin(eulerY * 0.5f), 0, cos(eulerY * 0.5f));
  Quaternion yaw(0, 0, sin(eulerZ * 0.5f), cos(eulerZ * 0.5f));
...
Vector3 Quaternion::GetEulerAngles() const {
  ...
  if (abs(sinPitch) >= 1)
    pitch =
        PI / 2 * Sign(sinPitch);  // use 90 degrees if out of range
  else
    pitch = asin(sinPitch);
...
void Quaternion::SetFromToRotation(const Vector3& fromDirection,
  ...
  float k =
      sqrt(fromDirection.SqrMagnitude() * toDirection.SqrMagnitude());
  if (abs(dot / k + 1) < EPSILON) {
    Vector3 ortho;
...
void Quaternion::SetLookRotation(const Vector3& forwardDirection,
  ...
  if (1 - abs(Vector3::Dot(forward, upwards)) < EPSILON)
...
float Quaternion::AngleRad(const Quaternion& aQuaternion,
  ...
  return acos(min(abs(dot), 1.f)) * 2.f;
}

float Quaternion::AngleDeg(const Quaternion& aQuaternion,
                           const Quaternion& bQuaternion) {
  return AngleRad(aQuaternion, bQuaternion) * RAD2DEG;
}
...
Quaternion Quaternion::Slerp(const Quaternion& aQuaternion,
  ...
  float theta{acos(Dot(aQuaternion, bQuaternion))};
  theta = abs(theta);
