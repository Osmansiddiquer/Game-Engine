...
void Transform::SetParent(Transform* const transform, bool inheritTransform) {
  ...
  if (transform == nullptr) {
    targetTransform = LevelManager::Instance().loadedLevel->levelRoot->transform;
  }
  if (parent == targetTransform) {
    ENGINE_WARN("You are trying to set (%s)'s parent to (%s), whose is already their parent",
...
Transform* Transform::GetChild(const U16 childIndex) {
  if (childIndex >= GetChildCount()) {
      std::stringstream ss; 
      ss << "Transform::GetChild => transform of " << GetName().c_str() << " only has " << GetChildCount() << " children but you are asking for the " << childIndex << "th one";
